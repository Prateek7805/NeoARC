...
function init(){
  ...
  Socket.onmessage = function(event){
    ...
    if(data[0] == "c"){
      ...
      doc("realtimeMode").checked = (json["realTime"] == 1)? true : false;
      changeRealtime(doc("realtimeMode").checked);
...
doc("realtimeMode").addEventListener('click', function(){
  changeRealtime(this.checked);
  send('CT',(this.checked)? 1 : 0);
});

function changeRealtime(cond){
  doc("brightness").removeEventListener((cond)? 'change' : 'input', changeBrightness);
  doc("brightness").addEventListener((cond)? 'input' : 'change', changeBrightness);
  doc("colorInput").removeEventListener((cond)? 'change' : 'input', changeColor);
  doc("colorInput").addEventListener((cond)? 'input' : 'change', changeColor);
}
